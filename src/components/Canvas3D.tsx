    ...
    const handler = (event: MouseEvent) => {
      ...
      dispatch(
        addBrick({
          ...
          scale: { x: type.size.x, y: type.size.z, z: type.size.y },
          locked: false,
          visible: true,
        })
      );
    };
    renderer.domElement.addEventListener('dblclick', handler);
    return () => renderer.domElement.removeEventListener('dblclick', handler);
  }, [rendererRef, dispatch, gridSnap, activeTool]);
